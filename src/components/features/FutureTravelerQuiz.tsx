...
const FutureTravelerQuiz = () => {
  ...
  const determineBadge = () => {
    // If no answers, return default badge
    if (answers.length === 0) {
    ...
    const maxAnswer = Object.entries(answerCounts).reduce(
      (maxEntry, currentEntry) => 
        currentEntry[1] > maxEntry[1] ? currentEntry : maxEntry
    )[0]; // [0] gets the key from the [key, value] tuple

    return badges[maxAnswer as keyof typeof badges] || badges.pioneer;
  };
  ...
  // Get the badge once and store it
            ...
            {!showResult ? (
              ...
              >
                ...
                <div className="bg-white/10 rounded-lg p-4 mb-8">
                  <p className="text-xl font-semibold text-white">{badge.discount}</p>
                </div>
                <motion.button
